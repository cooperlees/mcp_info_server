...
/// Bound on both the TCP connect and the RPC call for the Jaeger OTLP
/// exporter. Deliberately tight: `JAEGER_OTLP_ENDPOINT` normally reaches
/// Jaeger over a home VPN link from wherever this server runs, and this
/// server's own request handling must never wait on that link being up.
const JAEGER_EXPORT_TIMEOUT: Duration = Duration::from_secs(5);
...
///
/// If `JAEGER_OTLP_ENDPOINT` is set (e.g. `http://[fd00:68::25]:4317`),
/// every span this server already builds via `tracing` is additionally
/// exported to that endpoint over OTLP/gRPC — see `jaeger_tracer_provider`
/// for the non-blocking/failure-tolerant details. Returns the
...
pub fn init() -> Option<SdkTracerProvider> {
    ...
    // its own independent filter, so enabling Jaeger export doesn't require
    ...
    let tracer_provider = jaeger_tracer_provider();
...
/// Builds a Jaeger-bound tracer provider from `JAEGER_OTLP_ENDPOINT`, or
/// `None` if it's unset (checked here, not threaded through `Config`, so
/// this module stays self-contained the same way it already owns
/// `RUST_LOG`).
///
...
///   `JAEGER_EXPORT_TIMEOUT` — the connect bound matters specifically
///   because `opentelemetry-otlp`'s own `with_endpoint()`/`with_timeout()`
///   only ever set the RPC-call timeout, never `connect_timeout()`, so a
///   true network black hole (a dead VPN link — not connection-refused,
///   which fails fast on its own) would otherwise hang on tonic/hyper's
///   default (~30s) before the RPC-level timeout even applies. Building the
///   `Channel` by hand is the only way to set both.
/// - A malformed endpoint or exporter construction failure is logged once,
///   here, and just disables Jaeger export for this process rather than
...
///   installed.
fn jaeger_tracer_provider() -> Option<SdkTracerProvider> {
    let endpoint = std::env::var("JAEGER_OTLP_ENDPOINT").ok()?;

    let channel = match tonic::transport::Endpoint::from_shared(endpoint) {
        Ok(endpoint) => endpoint
            .connect_timeout(JAEGER_EXPORT_TIMEOUT)
            .timeout(JAEGER_EXPORT_TIMEOUT)
            .connect_lazy(),
        Err(e) => {
            eprintln!("JAEGER_OTLP_ENDPOINT is not a valid URI, disabling Jaeger export: {e}");
            return None;
        }
    };
        ...
        .build()
    {
        ...
        Err(e) => {
            eprintln!("failed to build Jaeger OTLP exporter, disabling Jaeger export: {e}");
            return None;
        }
    };
